# 🌊 Spiritual Voyage Design: 항해의 영적 치유 여정

> **"우려내는 데는 시간이 필요하다.  
> 빚는 데는 어둠이 필요하다."**
> 
> ---
> **"공간이 영혼을 치유하는 항해가 되다"**
> 
> 본 프로젝트는 항해의 은유를 통해 인간 내면의 깊은 치유와 변화를 안내하는 **융합 예술 공간 기획**입니다.

---

## 🧭 프로젝트 철학 (Philosophy)
영적 치유의 안내자(Shaman)로서, 방문객이 일상을 벗어나 자신의 내면으로 항해할 수 있는 '치유적 시공간'을 설계합니다. 

* **치유와 변화:** 공간에서의 경험이 삶의 변화로 이어지는 트리거가 됩니다.
* **항해의 은유:** 미지의 바다를 항해하듯, 공간의 요소를 통해 자아를 탐험하도록 돕습니다.

---

## 🖼️ Visual Gallery: 공간의 서사 (Core Concepts)

### **Part 1. The Canoe Table**

| | | |
| :---: | :---: | :---: |
| ![Canoe 1](canoe-table_1.png) | ![Canoe 2](canoe-table_2.png) | ![Canoe 3](canoe-table_3.png) |
| ![Canoe 4](canoe-table_4.png) | ![Canoe 5](canoe-table_5.png) | ![Canoe 6](canoe-table_6.png) |
| ![Canoe 7](canoe-table_7.png) | ![Canoe 8](canoe-table_8.png) | ![Canoe 9](canoe-table_9.png) |
| ![Canoe 10](canoe-table_10.png) | ![Canoe 11](canoe-table_11.png) | ![Canoe 12](canoe-table_12.png) |

### **Part 2. The Red Carpet**

| | | |
| :---: | :---: | :---: |
| ![Red 1](redcarpet_01.png) | ![Red 2](redcarpet_02.png) | ![Red 3](redcarpet_03.png) |
| ![Red 4](redcarpet_04.png) | ![Red 5](redcarpet_05.png) | ![Red 6](redcarpet_06.png) |
| ![Red 7](redcarpet_07.png) | ![Red 8](redcarpet_08.png) | |

### **Part 3. Nautical Interior**

| | | |
| :---: | :---: | :---: |
| ![Nau 1](nautical_01.png) | ![Nau 2](nautical_02.png) | ![Nau 3](nautical_03.png) |
| ![Nau 4](nautical_04.png) | ![Nau 5](nautical_05.png) | ![Nau 6](nautical_06.png) |
| ![Nau 7](nautical_07.png) | ![Nau 8](nautical_08.png) | ![Nau 9](nautical_09.png) |
| ![Nau 10](nautical_10.png) | ![Nau 11](nautical_11.png) | ![Nau 12](nautical_12.png) |
---

## 🛠️ 기획자 전문성 및 실적 (Credentials)

* **Creative Director:** Yeonjay Woo (우연재)
* **Design Methodology:** Midjourney AI 기반 비주얼 브랜딩 및 공간 시나리오 설계

### **[주요 프로젝트 실적]**
* **공공 인프라 및 콘텐츠 산업 기획:**
  - 광역자치단체 산하 디지털 혁신 거점 내 SW 및 콘텐츠 기업 지원 인프라 구축
  - 지역 문화예술 활성화를 위한 디지털 콘텐츠 통합 지원 체계 설계 및 운영
* **산학협력 및 창업 지원 성과:**
  - 대학 산학협력단 주관 창업 지원 프로젝트 선정 및 비즈니스 모델(BM) 고도화
  - 민-관-학 협업 기반의 문화 콘텐츠 융합 공간 기획 및 리딩
* **AI 테크 및 상업 브랜딩 실무:**
  - 전문가 플랫폼 내 AI 생성 모델 기반 제품 광고 기획 및 상업적 실용성 검증
  - [Nafeelme] 실제 제품 데이터 활용 AI 모델링 광고 콘텐츠 제작 프로젝트 운영
* **실증형 MICE 솔루션 운영:**
  - **[FUEL STAY / Mystic Brew]** 실증 공간 운영을 통한 정책 효과 및 공간 기획
  - 외국인 트래픽(Traffic) 연계 B2G MICE 솔루션 설계 및 여성기업 인증 보유

---

## 🔗 Quick Links
- [**디지털 포트폴리오 요약 (Links)**](https://yeonjay.github.io/links)

---

## ✉️ Contact
* **Portfolio:** [https://yeonjay.github.io](https://yeonjay.github.io)
* **Business:** AXIOM (MICE Solutions).
